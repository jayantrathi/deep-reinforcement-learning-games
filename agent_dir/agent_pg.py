...
class PolicyNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return action_prob
print("AgentPG class file is running")
class AgentPG(Agent):
...
def make_action(self, state, test=False):
    ...
    elif isinstance(state, (float, int, np.floating)):
        raise ValueError(f"Unexpected scalar state: {state} — expected vector input of shape (8,)")
...
def train(self):
    ...
    for epoch in range(self.num_episodes):
        ...
        if avg_reward > 100 and avg_reward > best_reward:
